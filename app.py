...
1. **Step 1: 요람(Curriculum) 기반 '수강 대상' 리스트 확정**:
   ...
   - **주의:** 'MSC 필수', '공학인증 필수'라고 적혀 있어도, 이 학기(예: 1학년 1학기) 표에 없으면 리스트에 넣지 마세요. (졸업 요건과 수강신청 대상은 다릅니다.)

2. **Step 2: 시간표 데이터와 정밀 대조 (Exact Match Only)**:
   - Step 1에서 확정된 리스트에 있는 과목만 시간표 데이터에서 찾으세요.
   - **과목명 완전 일치 필수**: 
     - 예: 요람에 '대학물리학1'이라고 되어 있다면, 시간표의 '대학물리및실험1'은 **다른 과목**으로 취급하고 제외하세요.
     - 글자 하나라도 다르면(특히 '및실험' 유무) 넣지 마세요.
   - **학년 불일치 자동 탈락 (Red Line)**:
     - 시간표 데이터의 '학년' 또는 '수강대상' 열을 반드시 확인하세요.
     - 사용자가 '1학년'인데 대상이 '2' 또는 '2학년'으로 되어 있다면, 과목명이 같아도 **절대** 넣지 마세요.

3. **출력 형식 (세로형 HTML Table)**:
   - 반드시 **HTML `<table>` 태그**를 사용해라.
   - **행(Row): 1교시 ~ 9교시**
     - 행 머리글(첫 번째 열)에 반드시 **시간 정보**를 포함해라. (예: 1교시 (09:00~10:15))
